...
def invert_delta_contraction(s_obs, p, sigma, R=200, max_iter=2000, tol=1e-10):
    """
    ...
    nu_draws = tfd.Normal(0.0, 1.0).sample(R, seed=123)
...
def gmm_objective_for_sigma(sigma, markets, iv_type="cost", R=200):
    ...
    delta_list = []
    for m in markets:
        delta_t = invert_delta_contraction(m["s"], m["p"], sigma, R=R)
